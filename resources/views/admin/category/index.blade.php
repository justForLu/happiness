    ...
    <div class="box">
        <div class="box-header">
            <form method="get" action="{{ url('admin/category') }}" class="form-horizontal" role="form">
                <div class="col-sm-2">
                    <input type="text" name="name" placeholder="分类名称" value="{{ isset($params['name']) ?  $params['name'] : ''}}" autocomplete="off" class="form-control">
                </div>
                <div class="col-sm-2">
                    {{\App\Enums\CategoryEnum::enumSelect(isset($params['type']) ?  $params['type'] : '','请选择类型','type')}}
                </div>
                <div class="col-sm-1">
